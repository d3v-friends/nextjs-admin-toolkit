...
for (let filename of brands) {
	...
	fs.writeFileSync(
		`../../src/asset/brand/${kebabCase(iconName)}.ts`,
		`
import  ${iconName} from "../../../asset/svg/brand/${filename}";
export {${iconName}};
`
	);
}
...
for (let filename of regulars) {
	...
	fs.writeFileSync(
		`../../src/asset/regular/${kebabCase(iconName)}.ts`,
		`
import  ${iconName} from "../../../asset/svg/regular/${filename}";
export {${iconName}};
`
	);
}
